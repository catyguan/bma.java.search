...
public class FileDictionaries implements Dictionaries {
	...
	public FileDictionaries(String dicHome, String skipPrefix,
			...
			String confucianFamilyName, String combinatorics,
			String charsetName, int maxWordLen) {
	...
	public synchronized Dictionary getVocabularyDictionary() {
		if (vocabularyDictionary == null) {
			...
			for (int i = 0; i < noiseWordsDic.size(); i++) {
				Hit hit = vocabularyDictionary.search(noiseWordsDic.get(i), 0,
						noiseWordsDic.get(i).length());
				if (hit.isHit()) {
			...
			for (int i = 0; i < noiseCharactorsDic.size(); i++) {
				Hit hit = vocabularyDictionary.search(
						noiseCharactorsDic.get(i), 0, noiseCharactorsDic.get(i)
								.length());
				if (hit.isHit()) {
	...
	protected synchronized void refreshDicWords(String dicPath) {
		...
		if (allWords != null) {
			try {
				Map<String, Set<Word>> temp = FileWordsReader.readWords(dicHome
						+ dicPath, charsetName, maxWordLen);
