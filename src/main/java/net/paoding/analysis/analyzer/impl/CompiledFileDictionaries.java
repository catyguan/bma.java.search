...
public class CompiledFileDictionaries implements Dictionaries {
	...
	protected Word[] getDictionaryWords(String dicNameRelativeDicHome) {
		...
		try {
			Map<String, Collection<Word>> map = FileWordsReader.readWords(
					f.getAbsolutePath(), charsetName, maxWordLen,
					LinkedList.class, ".dic.compiled");
			return map.values().iterator().next().toArray(new Word[0]);
		} catch (IOException e) {
