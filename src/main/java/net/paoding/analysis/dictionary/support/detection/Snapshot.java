...
public class Snapshot {
	...
	public Difference diff(Snapshot that) {
		...
		for (int i = 0; i < older.nodes.length; i++) {
			...
			InnerNode yongerNode = (InnerNode) younger.nodesMap
					.get((String) olderNode.path);
			if (yongerNode == null) {
		...
		for (int i = 0; i < younger.nodes.length; i++) {
			...
			InnerNode olderNode = (InnerNode) older.nodesMap
					.get((String) yongerNode.path);
			if (olderNode == null) {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		String deleted = ArraysToString(diff.getDeleted()
				.toArray(new Node[] {}));
		System.out.println("deleted: " + deleted);
		String modified = ArraysToString(diff.getModified().toArray(
				new Node[] {}));
		System.out.println("modified: " + modified);
		String newcome = ArraysToString(diff.getNewcome()
				.toArray(new Node[] {}));
